    ...
    def __init__(self,
        ...
        def random_shared(name):
            ...
            if not type(shape) == tuple:
                shape = (shape,)
            stop = reduce(lambda x, y: x*y, shape)
            return theano.shared(
                numpy.arange(stop).reshape(shape).astype(theano.config.floatX),
                name=name)
        ...
        self.names.remove('emb')  # no need to save or update embeddings TODO
        ...
        def recurrence(i,
            ...
            # eqn 9
            gru_input = T.concatenate(c, x_i, axis=1)
            h = GRULayer()
        ...
        outputs = recurrence(*outputs_info[2:])
...
if __name__ == '__main__':
    articles = numpy.arange(8, dtype='int32').reshape(2, -1)
    titles = numpy.arange(6, dtype='int32'). reshape(2, -1)
    rnn = Model()
